...
// ---------------------------------------------------------------------------
// AI Restoration Service Bridge (GFPGAN + Real-ESRGAN via Python sidecar)
// ---------------------------------------------------------------------------
...
async function callRestorationService(
  ...
  logger.info({ enhancementType, mode, serviceUrl: RESTORATION_SERVICE_URL }, "Calling restoration service");
...
): Promise<{ base64: string; mimeType: string; framesProcessed: number; processingMs: number }> {
  logger.info({ mode, faceEnhance, maxFrames }, "Calling video restoration service");
...
export async function enhanceImage(
  ...
  if (filterName && FILTER_PRESETS[filterName]) {
    ...
    switch (type) {
      ...
      // ── AI RESTORATION (GFPGAN + Real-ESRGAN via Python sidecar) ──────────
      case "face_restore":
      case "face_restore_hd":
      case "esrgan_upscale_2x":
      case "esrgan_upscale_4x":
      case "old_photo_restore": {
        const restorationResult = await callRestorationService(base64Data, type);
        return { base64: restorationResult.base64, mimeType: restorationResult.mimeType };
      }
      default: {
