...
router.post("/media/enhance", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  try {
    ...
    if (enhancementType === "video_restore") {
      const videoMode = (settings as Record<string, unknown> | undefined)?.mode as string ?? "upscale_2x";
      const faceEnhance = (settings as Record<string, unknown> | undefined)?.faceEnhance !== false;
      const result = await callVideoRestoration(rawB64, videoMode, faceEnhance);
      await db.update(mediaJobsTable).set({
        status: "completed",
        processedUrl: result.base64,
        thumbnailUrl: result.base64.slice(0, 1000), // video thumbnail: first bytes
        processingTimeMs: result.processingMs,
        completedAt: new Date(),
      }).where(eq(mediaJobsTable.id, jobId));
      logger.info({ jobId, type: enhancementType, frames: result.framesProcessed, ms: result.processingMs }, "Video restoration completed");
      return;
    }
