...
const SIMPLE_PRESETS: { type: EnhanceMediaBodyEnhancementType; label: string; desc: string; icon: React.ReactNode; filterName?: string }[] = [
  ...
  { type: "face_restore",           label: "Face Restore",     desc: "AI face restoration (GFPGAN)",        icon: <ScanFace     className="w-5 h-5" /> },
  { type: "esrgan_upscale_4x",      label: "AI Upscale 4×",    desc: "Real-ESRGAN super-resolution",        icon: <ImageUp      className="w-5 h-5" /> },
  { type: "old_photo_restore",      label: "Old Photo Fix",    desc: "Restore vintage & damaged photos",    icon: <History      className="w-5 h-5" /> },
];
...
export default function Editor() {
    ...
    if (currentJob.status === "completed" && processStage !== "completed") {
      ...
      if (studioMode === "photo" && currentJob.processedUrl) {
        saveToHistory({
          ...
          originalThumbnailUri: previewUrl || undefined,
          mimeType: file?.type ?? "image/jpeg",
          processingTimeMs: currentJob.processingTimeMs ?? undefined,
  ...
  const ENHANCEMENT_TYPES: { type: EnhanceMediaBodyEnhancementType; label: string; icon: React.ReactNode }[] = [
    ...
    { type: "face_restore_hd",        label: "Face HD",     icon: <ScanFace    className="w-3 h-3" /> },
    { type: "esrgan_upscale_2x",      label: "ESRGAN 2×",   icon: <ImageUp     className="w-3 h-3" /> },
    { type: "esrgan_upscale_4x",      label: "ESRGAN 4×",   icon: <ImageUp     className="w-3 h-3" /> },
    { type: "old_photo_restore",      label: "Old Photo",   icon: <History     className="w-3 h-3" /> },
  ];
